...
const (
	...
	updatePubkey     = `UPDATE pubkeys SET name = $3, body = $4 WHERE id = $1 AND account_id = $2`
...
func (di *pubkeyDaoSqlx) Update(ctx context.Context, pubkey *models.Pubkey) error {
	...
	res, err := stmt.ExecContext(ctx, pubkey.ID, ctxAccountId(ctx), pubkey.Name, pubkey.Body)
	if err != nil {
